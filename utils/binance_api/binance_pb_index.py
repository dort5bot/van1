"""
utils/binance_api/binance_pb_index.py
--------------------------------
...
FAPI_INDEX_INFO_ENDPOINT = "/api/v3/indexInfo"
FAPI_ASSET_INDEX_ENDPOINT = "/fapi/v1/assetIndex"
...
class BinancePBIndex:
    ...
    # -------------------------
    # Ek Index Endpoint'ler
    # -------------------------
    async def index_info(self, symbol: Optional[str] = None) -> Any:
        """
        Get index information.
        GET /api/v3/indexInfo
        """
        params = {"symbol": symbol.upper()} if symbol else None
        logger.debug("Index: index_info %s", symbol)
        data = await self._http.get(FAPI_INDEX_INFO_ENDPOINT, params=params)
        return data

    async def asset_index(self, symbol: Optional[str] = None) -> Any:
        """
        Get asset index for Multi-Assets mode.
        GET /fapi/v1/assetIndex
        """
        params = {"symbol": symbol.upper()} if symbol else None
        logger.debug("Index: asset_index %s", symbol)
        data = await self._http.get(FAPI_ASSET_INDEX_ENDPOINT, params=params, futures=True)
        return data
